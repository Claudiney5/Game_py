...
class Game(tk.Tk):
    def __init__(self):
        ...
        # Variáveis de ajuda, informações e dica aos jogadores:
        ...
        # inserindo na Lista de inventário os itens (variáveis de inventário):
        ...
        # criando um espaço entre os botões de inventário e preenchendo o X
        ...
        self.screen.bind("<Button-1>", self.handle_click, self.item_in_use)


    def handle_click(self, event, **item_in_use):
    ...
    def add_inventory_item(self, item_name):
        ...
        if next_available_inventory_slot < len(self.inventory_slots):
            ...
            next_label_var.set(item_name.title())  # coloca a primeira letra em maiuscula
    ...
    def clear_used_item(self):
    	...
    	for index, button in enumerate[self.inventory_slots]:
    		button.configure(bg="black")
    		use_cmd = partial(self.use_item, item_number=index)  # 'partial', fixa o argumento item_number do método use_item
    		button.configure(command=use_cmd)


    def show_cannot_use_message(self):
    	text_id = self.screen.create_text((250, 25), 
    		                              text="Este item não pode ser usado aqui!",
    		                              font=self.cannot_use_font,
    		                              fill="white")
...
if __name__ == "__main__":
    ...
    scene2 = GameScreen(game, "assets/scene2.png", (117, 54, 329, 412), None, "Você tem a chave!", "chave")
